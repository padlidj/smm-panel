            ...
            <TableBody>
              {tickets.map((t: any) => (
                <TableRow key={t.id}>
                  ...
                  <TableCell><Link href={`/dashboard/ticket/${t.id}`}><Button variant="secondary" size="sm">View</Button></Link></TableCell>
                </TableRow>
              ))}
              {tickets.length === 0 && <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground">No tickets found</TableCell></TableRow>}
            </TableBody>
          </Table>
        </CardContent>
        <Pagination page={page} totalPages={totalPages} onChange={p => router.push(`/dashboard/ticket/list?page=${p}${st ? `&status=${st}` : ''}`)} />
      </Card>
    </div>
  );
}