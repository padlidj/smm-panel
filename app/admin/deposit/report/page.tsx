...
export default async function AdminDepositReportPage({ searchParams }: { searchParams: { from?: string; to?: string; username?: string; status?: string } }) {
  await requireAdmin();
  ...
  const deposits = await prisma.deposit.findMany({
    ...
    select: { id: true, method: true, amount: true, fee: true, net: true, status: true, created_at: true, user: { select: { username: true } } },
  });

  const total = deposits.reduce((a, d) => a + Number(d.amount), 0);
  const totalNet = deposits.reduce((a, d) => a + Number(d.net), 0);
  const success = deposits.filter(d => d.status === 'SUCCESS');
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          { label: 'Berhasil', value: String(success.length) },
          { label: 'Total Nominal', value: `Rp ${total.toLocaleString('id-ID')}` },
          { label: 'Total Net Masuk', value: `Rp ${totalNet.toLocaleString('id-ID')}` },
        ].map(s => (
      ...
      <Card>
        <CardHeader><CardTitle className="text-lg">Detail Deposit</CardTitle></CardHeader>
        <CardContent className="p-0">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>ID</TableHead><TableHead>User</TableHead><TableHead>Metode</TableHead>
                <TableHead>Jumlah</TableHead><TableHead>Fee</TableHead><TableHead>Net</TableHead>
            ...
            <TableBody>
              ...
              {deposits.length === 0 && <TableRow><TableCell colSpan={8} className="text-center text-muted-foreground py-8">Tidak ada data pada rentang ini</TableCell></TableRow>}
            </TableBody>
          </Table>
        </CardContent>
      </Card>
